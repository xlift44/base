...
class Main extends Sprite {
    ...
    - never - доступ закрыт
    - get - должен быть создан соответствующий метод доступа (для переменной x метод должен именоваться get_x)
    - set - должен быть создан соответствующий метод доступа (для переменной x метод должен именоваться set_x)
    */

    // var prop0(default, never):Int; // прочитать можно, записать не получится
    // prop0 = 1;

    // var prop(get, null):Int = 7;
    // static function get_prop() { return prop + 1; }
    // trace("get_prop:", prop);
